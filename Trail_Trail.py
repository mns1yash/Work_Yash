...
optimizers = ['SGD(lr=0.0001, momentum=0.3, nesterov=True)',
              'Adam(lr=0.0001)',
              'Adagrad(lr=0.0001)',
              'RMSprop(lr=0.0001)']
...
for opt_name in optimizers:
    ...
    opt_model.compile(loss='binary_crossentropy',
                  ...
                  metrics=['accuracy'])
    opt_h = model.fit(X_train, y_train,
                  batch_size=16,
                  epochs=3,
                  verbose=0,
                  callbacks=[early_stop_loss],
...
print("optimizeList_acc",optimizeList_acc,
      "optimizeList_val",optimizeList_val,
      "optimizeList_loss",optimizeList_loss,
      "optimizeList_val_loss",optimizeList_val_loss)
#______________________________________________________________________________
#Display Ativation
...
activations = activation_model.predict(X_train[image_number])
...
# dataaug_h = model.fit(X_train, y_train,
#               batch_size=8,
#               epochs=25,
#               verbose=0,
#               callbacks=[early_stop_loss],
...
#               'RMSprop(lr=0.001)'], loc='best')
#   bottom, top = plt.ylim()  # return the current ylim
#   plt.ylim((bottom - bottom*0.03), (top + top*0.03))   # set the ylim to bottom, top# #_____________________________________________________________________________________________
# #Last Model
...
lr_model('lr_model.h5')
opt_model('opt_model.h5')
# dataaug_model('dataaug_model.h5')
# data_aug_model('data_aug_model.h5')
# aug_lr_model('aug_lr_model.h5')
# aug_opt_model('aug_opt_model.h5')
